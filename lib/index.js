...
Toast.install = function (Vue, options) {
    ...
    Vue.prototype.$toast = function (tips, type, cssCls) {
        ...
        cssCls = ['success', 'info', 'warning', 'danger'].find(function (cls) {
            if(cssCls === cls) {
                return cls;
            }
        });
        // Ensure it is not undefined
        cssCls = (cssCls === undefined) ? '' : cssCls;
            ...
            var toastTpl = Vue.extend({
                data: function () {
                    return {
                        show: showToast,
                        tip: tips,
                        type: 'lx-toast-' + curType + ' ' + cssCls // Additional class added
                    }
                },
                template: tmp
            });
        ...
        toastVM.type = 'lx-toast-' + curType + ' ' + cssCls; // Additional class added
        toastVM.tip = tips;
    ...
    ['bottom', 'center', 'top'].forEach(function (type) {
        Vue.prototype.$toast[type] = function (tips, cssCls) {
            return Vue.prototype.$toast(tips, type, cssCls)
        }
    });
